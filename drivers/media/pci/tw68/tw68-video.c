...
static int tw68_buf_prepare(struct vb2_buffer *vb)
{
	...
	struct tw68_dev *dev = vb2_get_drv_priv(vq);
	struct tw68_buf *buf = container_of(vb, struct tw68_buf, vb);
	struct vb2_dma_sg_desc *dma = vb2_dma_sg_plane_desc(vb, 0);
	unsigned size, bpl;
	int rc;
	...
	rc = dma_map_sg(&dev->pci->dev, dma->sglist, dma->num_pages, DMA_FROM_DEVICE);
	if (!rc)
		return -EIO;
	...
	switch (dev->field) {
	case V4L2_FIELD_TOP:
		tw68_risc_buffer(dev->pci, buf, dma->sglist,
